...
Tweeter.prototype.execute = function(path, options, cb) {
    ...
    self.log(self.logLevel.INFO, 'execute() headers: %j', options.headers );
...
Tweeter.prototype.log = function(level, msg) {
    try {
        ...
        stream.write( util.format.call(this, '[\033[%sm%s\033[39m]\t', colors[level[0]], level ) );
...
Tweeter.prototype.oauthSignature = function(method, path, props, query) {
    ...
    self.log(self.logLevel.DEBUG, 'Request: %s', sig);
    hmac.update(sig);
...
Tweeter.prototype.createSignatureBaseString = function(method, path, props, query) {
    ...
    if(props) {
        ...
        self.log(self.logLevel.INFO, 'Sorted array: %j', oauthProperties );
    }
